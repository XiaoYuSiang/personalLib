...
Line 258~259
   ...
   TreeReader(const char* patt,               const char* treename = "t");
   TreeReader(const char** paths, int npaths, const char* treename = "t");
   TreeReader(std::vector<std::string> paths, const char* treename = "t");
 
ggNtuplizer/EventTree
*/
...
std::vector<std::string> find_files(const char* patt)
{
   ...
   do {
	...
	std::cout<<"Find files:"<<base<<std::endl;
...
std::vector<std::string> find_filestxt(const char* patt)
{
   /* Returns sorted list of paths to root files matching the path pattern given.
    *
    * Documentation is given in the TreeReader(const char*, const char*)
   ...
   while (dirs.size() > 0) {
      ...
      while (fname = gSystem->GetDirEntry(dir)) {
         ...
         } else if (TString(fname).EndsWith(".txt")) {
            if (fnmatch(p, path, FNM_PATHNAME) == 0) {
//                Printf("TreeReader: adding %s", path.Data());
               paths.push_back(path.Data());
               // if(!path.Contains("job_DoubleEG_Run2016B_FebReminiAOD/ggtree_data_1637.txt")) paths.push_back(path.Data());
            }
         }
   ...
   Printf("find_filestxt(): %lu file%s found", paths.size(),
                                            paths.size() == 1 ? "" : "s");
